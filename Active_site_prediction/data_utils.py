...
class PrecomputedMCSADataset(torch.utils.data.Dataset):
    """
    Dataset that loads precomputed ESM-2 embeddings from memory-mapped files for the M-CSA test set.
    """
    def __init__(self, root="./data/test/precomputed_embeddings"):
        super().__init__()
    ...
    def open_memmaps(self):
        self._embeddings = np.memmap(
            ...
            shape=(self._total, self._max_length, self._hidden_dim)
        )
        self._masks = np.memmap(
            os.path.join(self._root, "masks.dat"),
            dtype=np.bool_, mode="r",
            shape=(self._total, self._max_length)
        )
...
class UniprotDataset(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, tokenizer, root="./data/uniprot/train_val_dataset_20.json", fold=None, max_length=1024):
        super().__init__()
        self.tokenizer = tokenizer
        self.max_length = max_length
